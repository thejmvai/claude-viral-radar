...
// 500557 -> "501K"; 1234567 -> "1.2M"; 940 -> "940"
...
export function formatDigest(dataset = {}, { top = 5, trends = 3, minPerHandle = 0 } = {}) {
  ...
  let text = lines.join("\n");
  if (text.length > TG_LIMIT) text = text.slice(0, TG_LIMIT - 1) + "…";
  return text;
}
...
async function main() {
  ...
  const dsPath = arg("dataset", path.join(outDir, `${niche}.json`));
  ...
  // minPerHandle from config when available (for the under-floor callout)
  let minPerHandle = Number(arg("min-per-handle", 0)) || 0;
  if (!minPerHandle) {
    const cfgPath = path.join(outDir, `${niche}.config.json`);
    if (fs.existsSync(cfgPath)) {
      try { minPerHandle = Number(JSON.parse(fs.readFileSync(cfgPath, "utf8")).minPerHandle) || 0; } catch {}
    }
  }

  const text = formatDigest(dataset, {
    top: Number(arg("top", 5)) || 5,
    trends: Number(arg("trends", 3)) || 3,
    minPerHandle,
  });
